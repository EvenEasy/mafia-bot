...
class function_buttons:
    ...
    async def StartTruthOrDare(self, interaction : discord.InteractionResponse):
        ...
        try:
            resp = await self.client.wait_for("interaction", check=lambda i:i.data["custom_id"] == "start_truth_or_dare" and interaction.user.id == i.user.id and interaction.message.id == i.message.id, timeout=600)
        except asyncio.TimeoutError:
        ...
        resp1 = await self.client.wait_for("interaction", check=lambda i:"language_" in i.data.get("values")[0] and interaction.user.id == i.user.id and interaction.message == i.message, timeout=600)
